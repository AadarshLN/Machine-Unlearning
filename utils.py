...
from wavemesh import wavemesh_superpixels
from graph_representation import superpixels_to_graph
from pooling_methods import wavepool
...
def get_dataset(data_name, path='./data'):
    if data_name not in ['mnist', 'cifar10']:
        raise TypeError('data_name should be a string, including mnist, cifar10.')
    ...
    if data_name == 'mnist':
        ...
        testset = datasets.MNIST(path, train=False, download=True, transform=transform)
    ...
    return trainset, testset #image stored as tensors to convert to image use a fn

def preprocess_image(image):
    image_np = np.array(image)

    labels, _ = wavemesh_superpixels(image_np, num_superpixels=100)
    graph = superpixels_to_graph(labels)
    pooled_graph = wavepool(graph)

    edge_index = torch.tensor(pooled_graph['edges'], dtype=torch.long)
    x = torch.tensor(pooled_graph['nodes'], dtype=torch.float)
...
def get_dataloader(trainset, testset, batch_size, device):
    def transform(data):
        if isinstance(data, (list,tuple)):
            image = data[0]
            target = data[1]
            graph_data = preprocess_image(image)
    ...
    train_loader = DataLoader(dataset=trainset, batch_size=batch_size, shuffle=True, collate_fn = lambda batch: [transform(data) for data in batch])
    test_loader = DataLoader(dataset=testset, batch_size=batch_size, shuffle=True, collate_fn = lambda batch: [transform(data) for data in batch])
...
def get_unlearn_loader(trainset, testset, forget_class, batch_size, num_forget, repair_num_ratio=0.01):
    ...
    train_forget_loader = DataLoader(dataset=trainset, batch_size=batch_size, sampler=train_forget_sampler)
    train_remain_loader = DataLoader(dataset=trainset, batch_size=batch_size, sampler=train_remain_sampler)
    repair_class_loader = DataLoader(dataset=trainset, batch_size=batch_size, sampler=repair_class_sampler)
    test_forget_loader = DataLoader(dataset=testset, batch_size=batch_size, sampler=test_forget_sampler)
    test_remain_loader = DataLoader(dataset=testset, batch_size=batch_size, sampler=test_remain_sampler)

    return train_forget_loader, train_remain_loader, test_forget_loader, test_remain_loader, repair_class_loader, train_forget_index, train_remain_index, test_forget_index, test_remain_index
...
def get_forget_loader(dt, forget_class):
    ...
    for i in range(len(dt)):
        ...
        if lbl == forget_class:
            ...
            els_idx.append(i)
    forget_loader = torch.utils.data.DataLoader(dt, batch_size=8, shuffle=False, sampler=torch.utils.data.SubsetRandomSampler(idx), drop_last=True)
    remain_loader = torch.utils.data.DataLoader(dt, batch_size=8, shuffle=False, sampler=torch.utils.data.SubsetRandomSampler(els_idx), drop_last=True)
    return forget_loader, remain_loader
