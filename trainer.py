...
from models import AllCNN,SimpleGNN
...
def train_gnn(model, train_loader, criterion, optimizer, device='cpu'):
    ...
    for batch in tqdm(train_loader, desc='Training', leave=False):
        if isinstance(batch, list):
            data = batch[0].to(device)  # Assuming the data is the first element in the list
        else:
            data = batch.to(device)  # For cases where batch is already a Data object
...
def train_save_model(train_loader, test_loader, model_name, optim_name, learning_rate, num_epochs, device, path):
    ...
    if model_name == 'AllCNN':
        ...
        for epo in range(num_epochs):
            ...
            if acc >= best_acc:
                ...
                #torch.save(model, '{}_epoch_{}.pth'.format(path, epo))
    ...
    if model_name == "GNN":
        ...
        train_loader, test_loader = get_dataloader(trainset, testset, batch_size=32, device='cuda')

        # Initialize model
        input_dim = 1024  # Example dimension, adjust based on your data
        hidden_dim = 512
        output_dim = 10  # Number of classes
        model = SimpleGNN(input_dim, hidden_dim, output_dim).to('cuda')
        ...
        for epoch in range(10):  # Number of epochs
            ...
            train_gnn(model,train_loader,criterion,optimizer,device)
            _, acc = eval(model=model, data_loader=test_loader, mode='', print_perform=False, device=device)
            print('test acc:{}'.format(acc))
            if acc >= best_acc:
                best_acc = acc
